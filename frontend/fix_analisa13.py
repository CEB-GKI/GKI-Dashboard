...
# 1. Remove analisa13Accum hook
content = re.sub(
    r"  const \[analisa13Accum, setAnalisa13Accum\] = useState\(false\);\n",
    "",
    content
)

# 2. Replace Analisa 13
analisa13_new = """  const analisa13 = useMemo(() => {
    ...
    const formatPeriode = (key: string) => {
      if (analisa13Time === '1m') {
        const [y, m] = key.split('-');
...
print("analisa13 fixed successfully")
